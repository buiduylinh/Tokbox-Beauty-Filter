...
import com.tokbox.sample.basicvideocapturercamera2.filter.utils.OpenGlUtils;
import com.tokbox.sample.basicvideocapturercamera2.filter.utils.ScaleTypeMatrix;
...
public abstract class AbsOesImageFilter implements GLSurfaceView.Renderer {
     ...
     * @return
     */
    public int createTexture(Image image, int mTextureId) {
        int[] texture = new int[1];
        byte[] bgraArray = bzyuvUtil.yuv420pToRGBA(image, false, 0);
        ByteBuffer byteBuffer = ByteBuffer.wrap(bgraArray);
        if (mTextureId == -1) {
            // 生成纹理，得到纹理id
            ...
            // 根据以上指定的参数，生成一个2D纹理
            GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, image.getWidth(), image.getHeight(),
                    0, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, byteBuffer);
        } else {
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, mTextureId);
            GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, image.getWidth(), image.getHeight(),
                    0, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, byteBuffer);
            texture[0] = mTextureId;
        }

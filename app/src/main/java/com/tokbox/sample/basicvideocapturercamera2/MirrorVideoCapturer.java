...
import com.tokbox.sample.basicvideocapturercamera2.filter.EGLRender;
import com.tokbox.sample.basicvideocapturercamera2.filter.MagicBeautyFilter;
...
class MirrorVideoCapturer extends BaseVideoCapturer implements BaseVideoCapturer.CaptureSwitch {
    ...
    private ImageReader.OnImageAvailableListener frameObserver = new ImageReader.OnImageAvailableListener() {
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            if (CameraState.CAPTURE == cameraState) {
                ...
                int captureWidth = frame.getWidth();
                int captureHeight = frame.getHeight();

                mEGLRender.setImage(frame);
                int[] outputFrame = mEGLRender.getRGBAArray();
                Log.d("LinhBD", (System.currentTimeMillis() - startTime) +"");
                provideIntArrayFrame(outputFrame, ABGR, captureWidth,
                        captureHeight, calculateCamRotation(), isFrontCamera());
            }
            frame.close();
        }
    };
    ...
    public MirrorVideoCapturer(Context context) {
        ...
        magicBeautyFilter = new MagicBeautyFilter(context);
        mEGLRender = new EGLRender(frameDimensions.getWidth(), frameDimensions.getHeight());
        mEGLRender.setFilter(magicBeautyFilter);
    }

    public void setBeautyLevel(float beautyLevel) {
        float beauty = beautyLevel * 2f - 100f;
        magicBeautyFilter.setBeautyLevel(beauty / 100f);
    }

    public void setToneLevel(float toneLevel) {
        float beauty = toneLevel * 2f - 100f;
        magicBeautyFilter.setToneLevel(beauty / 100f);
    }
    ...
    private void startCamThread() {
        ...
        cameraThread.start();
        cameraThreadHandler = new Handler(Looper.getMainLooper());
    }
